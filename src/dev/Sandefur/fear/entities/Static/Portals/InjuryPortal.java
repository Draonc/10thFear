...
import dev.Sandefur.fear.states.InjuryState;
import dev.Sandefur.fear.states.State;

public class InjuryPortal extends Portal {
	private Animation portal;

	public InjuryPortal(Handler handler, float x, float y) {
		super(handler, x, y);
		portal = new Animation(100, Assets.injuryPort);
	}

	@Override
	public void port() {
		if (touched && timer.run(time))
		State.setState(new InjuryState(handler));
	}
